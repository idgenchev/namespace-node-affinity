...
// CreateOrUpdate creates "namespace-node-affinity"
// mutating webhook configuration with "Ignore" failure policy for pods
// or returns an error
// NOTE: If the MutatingWebhookConfiguration already exists, the only thing that will be updated is the CABundle
func CreateOrUpdateMutatingWebhookConfig(k8sClient k8sclient.Interface, caBundle *bytes.Buffer, namespace, name, serviceName string) error {
	...
	if _, err := k8sClient.AdmissionregistrationV1().MutatingWebhookConfigurations().Create(context.Background(), mutateconfig, createOpts); err != nil {
		if k8serrors.IsAlreadyExists(err) {
			updateOpts := metav1.UpdateOptions{}
			_, err = k8sClient.AdmissionregistrationV1().MutatingWebhookConfigurations().Update(context.Background(), mutateconfig, updateOpts)
			return err
		}
		return err
	}
